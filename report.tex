...
\section{Paper Reading} 
	...
	\paragraph{Additional strategies for optimizing SGD} 1.Shuffling and Curriculum Learning,shuffle the training data after every epoch,框架已经内置，curriculum learning,example的顺序也十分重要，有论文显示train lstm 时候先简单后复杂有更好的训练结果。3.batch normalization 4.early stopping 5.Gradient noise，起初在梯度上加上一个均值为零，方差退火的噪声，发现可以使得网络发现不同的local minima，对poor initialization也表现出较好的鲁棒性。
\end{document}